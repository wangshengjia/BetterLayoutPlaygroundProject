...
class ReusableCollectionViewCell : UICollectionViewCell {
    ...
    func setupCellWithItem(item: AnyObject, reuseIdentifier: String) {
        if let configuration = Configurations.CellConfigurationWithReuseIdentifier(reuseIdentifier) {
            if let componentKeys = configuration[Configurations.CellKeys.components] as? Array<String> {
                if let componentStyles = configuration[Configurations.CellKeys.styles]  as? Array<String> {
                    var viewsDictionary = [String: UIView]()
                    for (index, componentKey) in componentKeys.enumerate() {
                        if let componentClass: AnyClass = Configurations.ComponentClasses[componentKey] as? CellComponentProtocol.Type as? AnyClass {
                    ...
                    if let visualFormats = configuration[Configurations.CellKeys.layout] as? Array<String> {
                        if let visualMetrics = configuration[Configurations.CellKeys.metrics] as? Dictionary<String, AnyObject> {
